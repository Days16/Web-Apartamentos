...
export default function PortalReserva() {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      generateInvoice({
        ...
        apt_slug: reservation.aptSlug || (apt as any).slug || '',
  ...
  const handleCancelReservation = async () => {
    if (!reservation || !apt) return;
    if (
      !window.confirm(T.common?.confirmCancel || '¿Estás seguro de que deseas cancelar la reserva?')
    )
      return;
    ...
    try {
      ...
      if (success) {
        ...
        // Notify user and manager by email
        await Promise.all([
  ...
  // Calculate if the reservation can be cancelled without penalty
  const canCancel = () => {
      ...
      <main className="flex-1 max-w-4xl mx-auto w-full px-4 py-12 md:py-20">
        <div className="max-w-md mx-auto mb-12 text-center">
          ...
          <p className="text-gray-600">
            Introduce tus datos para consultar el estado de tu estancia o descargar tu registro de reserva.
          </p>
        </div>
            ...
            <div className="bg-navy p-6 md:p-8 flex justify-between items-center text-white">
              ...
              <div className="text-right">
                ...
                <div className="text-lg font-mono font-bold text-teal">
                  {formatReservationReference(reservation.id, reservation.source)}
                </div>
              </div>
            </div>
            ...
            <div className="p-8 md:p-12">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex justify-between items-center border-b border-gray-100 pb-3">
                      ...
                      <span className="font-bold text-green-600 font-mono">
                        {formatPrice(reservation.deposit)} ✓
                      </span>
                    </div>
                    <div className="flex justify-between items-center bg-gray-50 p-4 rounded-lg mt-2 font-bold">
                      ...
                      <span className="text-navy">
                        {formatPrice(reservation.total - reservation.deposit)}
                      </span>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="flex items-start gap-3 mb-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-amber-700 mt-0.5">
                        Esta es la información disponible antes de las 48h de ingreso, para que
                        puedas revisarla con tranquilidad.
                      </p>
                    </div>
                  </div>
                  <div className="grid grid-cols-1 gap-3">
                    <div className="bg-white border border-amber-100 rounded-lg p-4">
                      ...
                      <p className="text-sm text-gray-700">
                        {settings?.property_address || 'Calle Illa Pancha 1, 27700 Ribadeo'}
                      </p>
                    </div>
                    <div className="bg-white border border-amber-100 rounded-lg p-4">
                      <p className="text-xs text-gray-500 uppercase font-bold tracking-wider mb-2">
                      ...
                      <p className="text-sm text-gray-700">
                        {settings?.checkin_access_info ||
                          'La caja de llaves está junto a la puerta principal. Introduce el código en el teclado numérico.'}
                      </p>
                    </div>
                    <div className="bg-white border border-amber-100 rounded-lg p-4">
                      <p className="text-xs text-gray-500 uppercase font-bold tracking-wider mb-2">
                      ...
                      <p className="text-sm text-gray-700">
                        {settings?.checkin_house_rules ||
                          'No se permiten mascotas. Horario de silencio de 23:00 a 08:00. Check-out antes de las 12:00.'}
                      </p>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400">
                        Información disponible para tu llegada
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div className="bg-teal/5 border border-teal/20 rounded-xl p-5">
                      ...
                      <p className="text-sm text-gray-700 leading-relaxed whitespace-pre-line">
                        {settings?.checkin_house_rules ||
                          'Consulta las normas en el tablón de bienvenida del apartamento.'}
                      </p>
                    </div>
                  </div>
                      ...
                      <div className="space-y-3">
                        {settings?.contact_phone && (
                          <a
                            ...
                            className="block text-lg font-bold text-navy hover:text-teal transition-colors"
                          >
                            {settings.contact_phone}
                          </a>
                        )}
                        {settings?.contact_whatsapp && (
                          <a
                            href={`https://wa.me/${settings.contact_whatsapp}?text=${encodeURIComponent(`Hola, soy ${reservation.guest} y tengo la reserva ${formatReservationReference(reservation.id, reservation.source)} para ${apt?.name} del ${reservation.checkin} al ${reservation.checkout}. Necesito ayuda.`)}`}
                            target="_blank"
                    ...
                    <div className="bg-gray-50 border border-gray-100 rounded-xl p-5">
                      ...
                      {settings?.property_address ? (
                        ...
                        <p className="text-sm text-gray-500">
                          El anfitrión te enviará las indicaciones de acceso por correo.
                        </p>
                      )}
                    </div>
                  </div>
              ...
              <div className="flex flex-col md:flex-row gap-4 pt-8 border-t border-gray-100">
                ...
                >
                  ...
                  Descargar Registro de Reserva PDF
                </button>
                {canCancel() ? (
                  <button
              ...
              {reservation.status === 'confirmed' && !canCancel() && (
                <div className="mt-4 text-center text-xs text-amber-600">
                  Periodo de cancelación gratuita superado. Contacta con nosotros para gestionar tu
                  reserva info@apartamentosillapancha.com.
                </div>
              )}
            </div>
          </div>
