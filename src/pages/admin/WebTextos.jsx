  ...
  return (
    <>
      <div className="pl-6 pr-6 py-6 border-b border-gray-200 bg-white">
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 border-b border-gray-200">
          <button
            ...
            className={`px-4 py-2 font-medium text-sm border-b-2 transition-colors whitespace-nowrap ${activeTab === 'inicio' ? 'border-[#1a5f6e] text-[#1a5f6e]' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
          >
            Inicio
          </button>
          <button
            onClick={() => setActiveTab('inicio_feat')}
            className={`px-4 py-2 font-medium text-sm border-b-2 transition-colors whitespace-nowrap ${activeTab === 'inicio_feat' ? 'border-[#1a5f6e] text-[#1a5f6e]' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
          >
            Inicio - Características
          </button>
          <button
            onClick={() => setActiveTab('nosotros')}
            className={`px-4 py-2 font-medium text-sm border-b-2 transition-colors whitespace-nowrap ${activeTab === 'nosotros' ? 'border-[#1a5f6e] text-[#1a5f6e]' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
          >
            Nosotros
          </button>
        </div>
      ...
      <div className="fixed bottom-0 right-0 left-0 bg-white border-t border-gray-200 py-4 px-6 z-50">
        <button
          className={`px-4 py-2 rounded-lg font-semibold transition-all cursor-pointer border-0 bg-teal-600 text-white hover:bg-teal-700 ${(!settings.home_hero_title?.trim() || !settings.email.includes('@') || !settings.phone.trim()) ? 'opacity-50' : 'opacity-100'}`}
          onClick={() => setConfirmOpen(true)}
          disabled={!settings.home_hero_title?.trim() || !settings.email.includes('@') || !settings.phone.trim()}
        >
          Guardar y publicar
        </button>
      </div>
