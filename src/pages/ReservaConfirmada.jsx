    ...
    if (!res) return (
        <div className="min-h-screen flex flex-col">
            ...
            <div className="flex-1 flex flex-center items-center justify-center p-8 text-center">
                <div>
                    <h1 className="text-3xl font-serif font-bold text-navy mb-4">Reserva no encontrada</h1>
                    <button onClick={() => navigate('/')} className="bg-teal text-white px-6 py-2 rounded">Volver al inicio</button>
                </div>
            </div>
            <Footer />
        </div>
            ...
            <main className="flex-1 max-w-3xl mx-auto w-full px-4 py-12 md:py-20">
                <div className="bg-white rounded-2xl shadow-xl overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <div className="bg-teal-600 p-8 text-white text-center">
                        ...
                        <h1 className="text-3xl md:text-4xl font-serif font-bold mb-2">¡Reserva Confirmada!</h1>
                        <p className="text-teal-50 opacity-90">Hola {res.guest}, tu estancia en Illa Pancha está lista.</p>
                    </div>

                    <div className="p-8 md:p-12">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
                            <div>
                                <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-4">Detalles de la Estancia</h3>
                                <div className="space-y-4">
                                    ...
                                    <div className="flex items-start gap-3">
                                        ...
                                        <div>
                                            <div className="text-sm font-bold text-navy">{res.checkin} — {res.checkout}</div>
                                            <div className="text-xs text-gray-500">{res.nights} {res.nights === 1 ? 'noche' : 'noches'}</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div>
                                <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-4">Información de Pago</h3>
                                <div className="space-y-4">
                                    ...
                                    <div className="flex justify-between items-center bg-green-50 p-3 rounded-lg border border-green-100">
                                        <span className="text-xs text-green-700">Depósito Pagado</span>
                                        <span className="font-bold text-green-700">{formatPrice(res.deposit)} ✓</span>
                                    </div>
                                    {res.total > res.deposit && (
                                        <div className="flex justify-between items-center p-3">
                                            <span className="text-xs text-gray-500">Pendiente al llegar</span>
                                            <span className="font-bold text-navy">{formatPrice(res.total - res.deposit)}</span>
                                        </div>
                                    )}
                                </div>
                            </div>
                        ...
                        <div className="border-t border-gray-100 pt-8 flex flex-col gap-4">
                            ...
                            <p className="text-center text-xs text-gray-400 mt-4 leading-relaxed">
                                Hemos enviado todos los detalles y el código de confirmación <strong>{res.id}</strong> a su email.
                                Si tiene alguna duda, contáctenos en info@illapancha.com
                            </p>
                        </div>
                    </div>
                </div>
