...
export default function WhatsAppButton() {
  ...
  useEffect(() => {
    fetchSettings().then(set => {
      if (set && set.site_phone) setPhone(set.site_phone.replace(/\D/g, ''));
    });
  }, []);

  if (!phone) return null;

  const message = encodeURIComponent('Hola, me gustaría información sobre los apartamentos en Ribadeo.');
